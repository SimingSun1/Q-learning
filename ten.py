...
class ALPaCAModel(tf.keras.Model):
    def __init__(self, config, preprocess=None, f_nom=None):
        ...
        self.SigEps = tf.Variable(np.array(config['sigma_eps']), dtype=tf.float32, name='SigEps') if isinstance(config['sigma_eps'], list) else config['sigma_eps'] * tf.eye(config['y_dim'])
    ...
    def compute_pred_and_nll(self, phi, posterior_K, posterior_L_inv, y=None, f_nom_x=None):
...
class ALPaCA:
    def __init__(self, config, preprocess=None, f_nom=None):
        self.config = deepcopy(config)
        self.model = ALPaCAModel(config, preprocess, f_nom)
        ...
        with self.summary_writer.as_default():
            for i in range(num_train_updates):
                ...
                with tf.GradientTape() as tape:
                    ...
                    self.phi = self.model.call(self.x, training=True)
                    self.context_phi = self.model.call(self.context_x, training=True)
                    ...
                    self.mu_pred, self.Sig_pred, self.predictive_nll = self.model.compute_pred_and_nll(self.phi, self.posterior_K, self.posterior_L_inv, self.y)
